...
func NewDefaultMounter(dir string, options ...fuse.MountOption) DefaultMounter {
	return DefaultMounter{dir: dir, options: options}
}

// Mount uses default mount
func (m DefaultMounter) Mount() (*fuse.Conn, error) {
	return fuseMountDir(m.dir, m.options)
}
...
func NewForceMounter(dir string, options ...fuse.MountOption) ForceMounter {
	return ForceMounter{dir: dir, options: options}
}

// Mount tries to mount and then unmount, re-mount if unsuccessful
func (m ForceMounter) Mount() (*fuse.Conn, error) {
	c, err := fuseMountDir(m.dir, m.options)
	if err == nil {
		return c, nil
	}
...
func fuseMountDir(dir string, options []fuse.MountOption) (*fuse.Conn, error) {
	psoptions, err := getPlatformSpecificMountOptions(dir)
	if err != nil {
		return nil, err
	}
	c, err := fuse.Mount(dir, append(options, psoptions...)...)
	if err != nil {
		err = translatePlatformSpecificError(err)
		return nil, err
	}
	return c, nil
}
